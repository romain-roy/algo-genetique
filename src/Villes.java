...
public class Villes {
	...
	public static void lireVilles(String fichier) {
		...
		try {
			...
			while ((ligne = br.readLine()) != null){
				...
				coords.add(new Ville(Double.parseDouble(ligne.substring(0,
						distToCut)), Double.parseDouble(ligne.substring(
						distToCut + 1, ligne.length()))));
			...
			System.out.println(e.toString()
					+ " : Erreur lors de l'ouverture du fichier des villes");
		}
		System.out.println(coords);
	}
	...
	public static double distance(Ville ville1, Ville ville2) {
		return Math.sqrt(Math.pow(ville2.getCoordX() - ville1.getCoordX(), 2)
				+ Math.pow(ville2.getCoordY() - ville1.getCoordY(), 2));
	}
